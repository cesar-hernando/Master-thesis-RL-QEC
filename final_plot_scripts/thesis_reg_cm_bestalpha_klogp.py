...
ax.plot(PS, prior, "o-", color=C_PRIOR, lw=1.8, ms=6, alpha=0.6,
        label=r"prior edge  ($\propto\log\frac{1}{p}$)")
ax.plot(PS, post1, "s-", color=C_CM, lw=1.8, ms=6, alpha=0.6, label=r"CM ($\alpha=1$)")
ax.plot(PS, posta, "D", color=C_RCM, ms=8, label=r"RCM ($\alpha^{*}$)")
pg = np.geomspace(PS[lo].min(), PS[lo].max(), 50)
ax.plot(pg, k1 * np.log(1.0 / pg) + c1, "-", color=C_RCM, lw=2.4,
