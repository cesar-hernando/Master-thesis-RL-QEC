...
def _finish(fig, axes, distances, ncols, nrows, ylabel):
    ...
    for i, ax in enumerate(used):
        ax.text(-0.17, 1.04, f"({labs[i]})", transform=ax.transAxes, fontsize=13, fontweight="bold")
    for ax in axes.flat:
        if ax not in used:
...
def make_ler(distances, fname):
    ...
    with mpl.rc_context({"font.size": 12, "axes.labelsize": 14, "axes.titlesize": 14,
                         "legend.fontsize": 11}):
        fig, axes = plt.subplots(nrows, ncols, figsize=(3.7 * ncols, 3.5 * nrows),
...
def make_alpha(distances, fname):
    ...
    with mpl.rc_context({"font.size": 12, "axes.labelsize": 14, "axes.titlesize": 14}):
        ...
        for k, d in enumerate(distances):
            ...
            ax.plot(pb, ab, marker="s", color=c, ls="-", mec="white", mew=0.6, lw=1.8)
            ax.set_title(ts.DLABEL[d]); ax.set_xscale("log"); ax.set_xlim(6e-5, 1.3e-2)
            ax.set_ylim(0, 1.0)
