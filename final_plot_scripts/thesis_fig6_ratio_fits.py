...
def finish(fig, axes, distances, ncols, nrows, ylabel):
    ...
    for i, ax in enumerate(used):
        ax.text(-0.19, 1.04, f"({string.ascii_lowercase[i]})", transform=ax.transAxes,
...
def make(distances, suffix):
    ...
    ptop, pbot = {}, {}
    with mpl.rc_context({"font.size": 11, "axes.labelsize": 13, "axes.titlesize": 13,
                         "legend.fontsize": 8.6}):
        figA, axesA = plt.subplots(nrows, ncols, figsize=(3.6 * ncols, 3.5 * nrows), squeeze=False)
        figB, axesB = plt.subplots(nrows, ncols, figsize=(3.6 * ncols, 3.5 * nrows),
    ...
    # ---------------- coefficients table ----------------
    figt, axt = plt.subplots(2, 1, figsize=(8.4, 1.7 + 0.66 * len(distances)))
    ...
    tb0.auto_set_font_size(False); tb0.set_fontsize(9.5)
    ...
    tb1 = axt[1].table(cellText=cells_bot, colLabels=col_bot, loc="center", cellLoc="center",
                       bbox=[0.0, 0.0, 1.0, 0.82])
    tb1.auto_set_font_size(False); tb1.set_fontsize(9.5)
