...
a5.plot(z["P5"], z["cm5"], "s-", color=C_CM, label=r"CM ($\alpha=1$)")
a5.plot(z["P5"], z["mw5"], "o--", color=C_MW, label="MWPM")
a5.set_title(r"$d=5$: weight-2 configurations")
a5.set_ylabel("Confusing configurations")

a7.errorbar(z["P7"], z["cm7"] / 1e3, yerr=z["cm7e"] / 1e3, marker="s", ls="-", color=C_CM,
            capsize=3, elinewidth=1.0, label=r"CM ($\alpha=1$)")
a7.plot(z["P7"], z["mw7"] / 1e3, "o--", color=C_MW, label="MWPM")
