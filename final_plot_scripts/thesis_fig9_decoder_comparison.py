...
def plot_method(ax, key, x, y, e):
    c, mk, lab = M[key]
    ax.errorbar(x, y, yerr=e, marker=mk, ls="-", color=c, ms=5.5, mec="white", mew=0.6,
                capsize=2.2, elinewidth=0.9, lw=1.6, label=lab)


def make(with_ncm):
    fig, (a5, a7) = plt.subplots(1, 2, figsize=(9.4, 4.3))
    for ax in (a5, a7):
        ax.axhline(1.0, color="0.55", lw=1.0, ls=(0, (4, 3)), zorder=1, label="MWPM")
    # d=5
    ...
    for ax, lab in zip((a5, a7), "ab"):
        ...
        ax.text(-0.17, 1.02, f"({lab})", transform=ax.transAxes, fontsize=12, fontweight="bold")
    h, l = a5.get_legend_handles_labels()      # a5 carries all methods (+ MWPM line)
    fig.tight_layout(w_pad=2.2)
    fig.subplots_adjust(top=0.85)
    leg = fig.legend(h, l, loc="upper center", ncol=len(l), frameon=True, framealpha=1.0,
                     edgecolor="0.7", fontsize=10, bbox_to_anchor=(0.5, 1.0))
    leg.get_frame().set_linewidth(0.8)
    ts.save(fig, "decoder_comparison_ler_d5_d7" + ("" if with_ncm else "_no_ncm"))


def make_d5(with_ncm):
    fig, ax = plt.subplots(figsize=(5.8, 4.6))
    ax.axhline(1.0, color="0.55", lw=1.0, ls=(0, (4, 3)), zorder=1, label="MWPM")
    plot_method(ax, "regcm", *scan_ratio(5, "best"))
    plot_method(ax, "cm", *scan_ratio(5, "cm1"))
    plot_method(ax, "bm", *bm_ratio(5))
    plot_method(ax, "tess", *tess_ratio())
    if with_ncm:
        plot_method(ax, "ncm", *ncm_ratio())
    ax.set_xscale("log")
    ax.set_xlabel(r"Physical error rate $p$")
    ax.set_ylabel(r"$p_{\mathrm{L}}\,/\,p_{\mathrm{L}}^{\mathrm{MWPM}}$")
    ax.set_title(r"$d=5$")
    ...
    leg = fig.legend(h, l, loc="upper center", ncol=3, frameon=True, framealpha=1.0,
                     edgecolor="0.7", fontsize=9, bbox_to_anchor=(0.5, 1.0))
    leg.get_frame().set_linewidth(0.8)
    ts.save(fig, "decoder_comparison_ler_d5" + ("" if with_ncm else "_no_ncm"))


ts.set_style(base=9.5)
make(with_ncm=True)
make(with_ncm=False)
make_d5(with_ncm=True)
make_d5(with_ncm=False)
