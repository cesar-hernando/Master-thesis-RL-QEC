...
def make(distances, fname, nrows=1):
    ...
    with mpl.rc_context({}):
        fig, axes = plt.subplots(nrows, ncols,
                                 figsize=ts.figsize(nrows, ncols, panel_ratio=0.92, extra_h=0.55),
        ...
        for k, d in enumerate(distances):
            ...
            for a in ALPHAS:
                ...
                ax.plot(xs, ys, marker="o", ms=3.4, ls="none", color=cmap(norm(a)), zorder=2)
            ax.set_xscale("log")
        ...
        for i, d in enumerate(distances):
            r, c = i // ncols, i % ncols
            ts.panel(axes[r, c], labs[i], x=-0.24 if c == 0 else -0.12, y=1.05)
