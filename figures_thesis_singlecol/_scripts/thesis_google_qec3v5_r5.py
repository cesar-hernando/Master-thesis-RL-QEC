...
ts.set_style()
fig, ax = plt.subplots(figsize=ts.figsize(1, 1, panel_ratio=0.82))
ys = np.arange(len(rows))[::-1]                     # first row at top
for y, (lab, l, se, c, mk) in zip(ys, rows):
    ax.errorbar(l, y, xerr=se, marker=mk, ms=6, color=c, mec="white", mew=0.7,
                capsize=2.2, elinewidth=1.0, zorder=3)
    ax.annotate(f"{l*100:.2f}\\%" if ts.USETEX else f"{l*100:.2f}%", (l + se, y),
                xytext=(6, 0), textcoords="offset points", va="center", ha="left",
                fontsize=ts.BASE - 1, color="0.25")
# faint guide at the belief-matching value
ax.axvline(rows[-1][1], color=C_BM, lw=0.9, ls=(0, (4, 3)), alpha=0.5, zorder=1)
