"""
Evaluate three decoders on Google Willow experimental data (d=3, d=5, d=7):
  1) Standard MWPM with calibrated weights.
  2) Correlated Matching (manual two-pass with calibrated joints if calibration ran;
     otherwise pymatching's native enable_correlations=True on the analytical DEM).
...
Data layout (as actually shipped in the Willow zip):
    <data-root>/d{D}_at_q{patch}/{X|Z}/r{N}/
        circuit_ideal.stim
        circuit_noisy_si1000.stim     <- used to build DEM (SI1000 prior)
        detection_events.b8           <- 1 bit per detector, b8-packed
        obs_flips_actual.b8           <- 1 bit per shot (one observable)
        metadata.json
        decoding_results/<pathway>/{error_model.dem, obs_flips_predicted.b8}

Patches per distance: d3 has 9, d5 has 4, d7 has 1 (q6_7).
Rounds: 1, 10, 13, 30, 50, 70, 90, 110, 130, 150, 170, 190, 210, 230, 250.
50000 shots per file. XZZX surface code (NOT standard rotated).

The script writes two CSVs by default for downstream plotting via
plot_experimental_results.py:
    data/experimental/willow_results.csv          (one row per evaluation)
    data/experimental/willow_calibration_trace.csv (one row per calibration iter)
"""
...
DATASET_NAME = "willow_105Q"
DEFAULT_PATCH = {3: "q6_7", 5: "q6_5", 7: "q6_7"}


# =============================================================================
# Experimental data generator: drops Stim sampling, reads real shots from disk.
# =============================================================================
...
class ExperimentalDataGenerator(SyndromeDataGenerator):
    ...
    without modifying the env. All three core methods are overridden:
      - generate_base_circuit   : returns the experimental Stim circuit + decomposed DEM
      - generate_drifted_circuit: no synthetic drift (mismatch=1.0)
      - simulate_syndrome_data  : slices preloaded experimental data
    """
    ...
    def __init__(
        ...
        # Placeholder fields the parent expects; we override the methods that use them.
    ...
    def _build_dem(self):
        """Decomposed DEM. The project decomposer assumes rotated-surface-code coords
        from stim.Circuit.generated; Willow's XZZX layout breaks it, so we fall back
        to Stim's built-in decompose_errors=True.
        """
        try:
...
def _compute_p_joint_from_decodes(edges_batch: np.ndarray, line_edge_index: np.ndarray) -> np.ndarray:
    """P(e_i AND e_j) co-occurrence rate over the calibration MWPM decodes."""
...
def resolve_data_root(data_root: str) -> str:
    inner = os.path.join(data_root, "google_105Q_surface_code_d3_d5_d7")
    return inner if os.path.isdir(inner) else data_root


def list_patches(data_root: str, distance: int):
    data_root = resolve_data_root(data_root)
    hits = sorted(glob(os.path.join(data_root, f"d{distance}_at_*")))
    return [os.path.basename(h).replace(f"d{distance}_at_", "") for h in hits]


def find_experiment_dir(data_root: str, distance: int, rounds: int,
                        basis: str, patch: str) -> str:
    data_root = resolve_data_root(data_root)
    basis_dir = basis.upper()
    round_dir = f"r{rounds:02d}" if rounds < 10 else f"r{rounds}"
    path = os.path.join(data_root, f"d{distance}_at_{patch}", basis_dir, round_dir)
    if not os.path.isdir(path):
        available_patches = list_patches(data_root, distance)
        raise FileNotFoundError(
            f"Could not find {path}.\n"
            f"  Available patches for d={distance}: {available_patches}\n"
            f"  Available rounds: 1, 10, 13, 30, 50, 70, 90, 110, 130, 150, 170, 190, "
            f"210, 230, 250."
        )
    return path
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate three decoders on Google Willow experimental data."
    )
    parser.add_argument("--data-root", default=None,
                        help="Root of the extracted Willow dataset (either the outer "
                             "wrapper or the dataset folder itself).")
    ...
    parser.add_argument("--patch", default=None,
                        help="Patch identifier (e.g. 'q6_5'). Default depends on distance.")
    ...
    parser.add_argument("--results-csv", default="data/experimental/willow_results.csv")
    parser.add_argument("--trace-csv", default="data/experimental/willow_calibration_trace.csv")
    ...
    if args.list_patches:
        if args.data_root is None:
            raise SystemExit("Provide --data-root to list patches.")
        print(f"Patches available for d={args.distance}: "
              f"{list_patches(args.data_root, args.distance)}")
        return
    ...
    if args.circuit_path and args.dets_path and args.obs_path:
        ...
        patch = args.patch or "custom"
    else:
        if args.data_root is None:
            raise SystemExit("Provide either --data-root or all three of "
                             "--circuit-path / --dets-path / --obs-path.")
        patch = args.patch or DEFAULT_PATCH[args.distance]
        exp_dir = find_experiment_dir(args.data_root, args.distance, args.rounds, basis, patch)
        circuit_path = args.circuit_path or os.path.join(exp_dir, "circuit_noisy_si1000.stim")
        dets_path = args.dets_path or os.path.join(exp_dir, "detection_events.b8")
        obs_path = args.obs_path or os.path.join(exp_dir, "obs_flips_actual.b8")

    print(f"[Willow] d={args.distance}  rounds={args.rounds}  basis={basis}  patch={patch}")
    ...
    print("\n" + "=" * 72)
    print(f"Results  d={args.distance}  rounds={args.rounds}  basis={basis}  patch={patch}")
